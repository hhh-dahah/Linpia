...
export default async function ProfilePage({ searchParams }: ProfilePageProps) {
  ...
  const showcase = await getPersonalShowcase(flow.user.id, flow.role, "session");

  if (!showcase) {
    redirect(
      flow.role === "student"
        ? `/profile/student?next=${encodeURIComponent(nextPath)}`
        : `/profile/mentor?next=${encodeURIComponent(nextPath)}`,
    );
  }
  ...
  return (
    <FormShell
      ...
      description="这里会集中展示你的基础信息、方向、标签和联系方式。完善好资料后，首页人才池和个人展示页都会同步更新。"
