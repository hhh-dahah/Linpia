  ...
  return (
    <article className="surface-card rounded-[1.75rem] p-6 hover:-translate-y-1">
      <div className="flex items-start gap-4">
        <div className="flex h-14 w-14 shrink-0 items-center justify-center rounded-2xl bg-[linear-gradient(135deg,_rgba(51,112,255,0.14),_rgba(116,194,255,0.22))] text-lg font-bold text-primary-strong">
          {item.name.slice(0, 1)}
        </div>
      ...
      <p className="mt-4 text-sm leading-7 text-muted">{item.bio || "这个同学还在补充个人介绍。"}</p>
      ...
      <div className="mt-5 flex flex-wrap gap-2">
        {visibleSkills.length ? (
          visibleSkills.map((skill) => (
            <span
              key={skill}
              className="rounded-full bg-[rgba(51,112,255,0.08)] px-3 py-1 text-xs font-medium text-primary-strong"
            >
              {skill}
            </span>
          ))
