...
export function LoginForm({ nextPath }: { nextPath: string }) {
  ...
  async function handleRequestCode(event: React.FormEvent<HTMLFormElement>) {
    ...
    startTransition(async () => {
      ...
      setServerState({
        status: "success",
        message: `8 位验证码已发送到你的邮箱，请输入 ${OTP_LENGTH} 位验证码完成登录。`,
      });
    });
  }
  ...
  async function handleVerifyCode(event: React.FormEvent<HTMLFormElement>) {
    ...
    if (!/^\d{8}$/.test(normalizedCode)) {
      nextErrors.code = "请输入 8 位验证码。";
    }

    if (Object.keys(nextErrors).length > 0) {
      setFieldErrors(nextErrors);
      setServerState({ status: "error", message: "请先填写正确的邮箱和 8 位验证码。" });
      scrollToFirstError(nextErrors);
      return;
    }
    ...
    startTransition(async () => {
      ...
      if (error) {
        setServerState({ status: "error", message: "验证码错误或已过期，请重新获取。" });
        return;
      }

      setServerState({ status: "success", message: "登录成功，正在返回下一步。" });
