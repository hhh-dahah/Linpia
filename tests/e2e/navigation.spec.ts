...
test.describe("导航与人才池", () => {
  test("顶部导航包含首页、找队伍、发招募、人才池、个人资料", async ({ page }) => {
    ...
    await expect(page.locator('nav a[href="/talent"]').first()).toBeVisible();
    await expect(page.locator('nav a[href="/profile"]').first()).toBeVisible();
  });

  test("首页包含人才池模块，导师区在学生区上方", async ({ page }) => {
    await page.goto("/");
  ...
  test("点击人才池导航会进入独立人才池页面", async ({ page }) => {
    await page.goto("/opportunities");
    await page.locator('nav a[href="/talent"]').click();
